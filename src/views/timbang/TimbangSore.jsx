...
function TimbangSore() {
  ...
  return (
    <>
      <h2 className="intro-y text-lg font-medium mt-10">Timbang Sore</h2>
...
function ModalTalang({ setTalangModal, talangModal, syncData }) {
  ...
  function handleTimbang(data) {
    let beratSore = 0;
    syncData?.map((item) =>  {
      beratSore += parseFloat(item.berat)
    })
    let temp = {
      ...data,
      berat_sore: beratSore,
      jumlah_barang_sore: syncData?.length,
    }
    simpanTimbang(temp)
  }
