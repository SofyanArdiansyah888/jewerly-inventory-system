...
import { useRiwayatAktivitas } from "../../hooks/useHistory";
import { helper } from "../../utils/helper";

function RiwayatAktifitas() {
  const [date, setDate] = useState();
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState("");
  const { data, isFetching } = useRiwayatAktivitas(page);
  const filterData = () => {
    return data?.data.filter(
      (item) =>
        item.user.toLocaleLowerCase().includes(search.toLocaleLowerCase()) ||
        item.activity.toLocaleLowerCase().includes(search.toLocaleLowerCase())
    );
  };
  return (
    <>
      <h2 className="intro-y text-lg font-medium mt-10">Riwayat Aktivitas</h2>
      <div className="grid grid-cols-12 gap-6 mt-5">
            ...
            <table className="table table-report -mt-2">
              <thead>
                <tr>
                  ...
                  <th className="text-center whitespace-nowrap">Aktifitas</th>
        ...
        {filterData()?.length > 9 && <nav className="w-full sm:w-auto sm:mr-auto">
