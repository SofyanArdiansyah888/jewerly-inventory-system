...
import DeleteModal from "../Penjualan/Delete"
export default function Index() {
  const [search, setSearch] = useState("");
  const { id: idBox } = useParams();
  const { data: penjualans, isLoading: loading } = usePenjualan(idBox);
  ...
  const filterData = () => {
    return penjualans?.data?.filter((item) =>
      item.nomor_invoice
        .toLocaleLowerCase()
  ...
  return (
    <>
      <h2 className="intro-y text-lg font-medium mt-10">Data Penjualan</h2>
      <div className="grid grid-cols-12 gap-6 mt-5">
        <div className="intro-y col-span-12 flex flex-wrap sm:flex-nowrap justify-between mt-2">
          <button
            ...
            onClick={() => navigate("/penjualan/create")}
          >
                  ...
                  <tr key={key} className="intro-x">
                    ...
                    <td>
                      <div className="whitespace-nowrap capitalize">
                        {box.nomor_invoice ? box.nomor_invoice : "-"}
                      </div>
                    </td>
                    {/* TANGGAL */}
                    <td>
