...
function Index() {
  ...
  const { data: rekenings, isLoading: loading } = useRekenings();
  ...
  const handleDelete = (rekening) => {
    setselectedRekening(rekening);
    setModalDelete(true);
  };

  function handleDetail(rekening) {}

  const filterData = () => {
    return rekenings?.filter(
      (item) =>
        item.nama_bank
          .toLocaleLowerCase()
          .includes(search.toLocaleLowerCase()) ||
        item.norek.toLocaleLowerCase().includes(search.toLocaleLowerCase())
    );
  };

  return (
    <>
      <h2 className="intro-y text-lg font-medium mt-10">Rekening</h2>
        ...
        <div className="intro-y col-span-12 flex flex-wrap sm:flex-nowrap justify-between mt-2">
          ...
          <div className="w-full sm:w-auto mt-3 sm:mt-0 sm:ml-auto md:ml-0">
            <div className="w-56 relative text-slate-500">
              <input
                type="text"
                className="form-control w-56 rekening pr-10"
                placeholder="Search..."
            ...
            <table className="table table-report -mt-2">
              <thead>
                <tr>
                  ...
                  <th className="whitespace-nowrap">Nama Bank</th>
                  <th className="text-center whitespace-nowrap">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filterData()?.map((rekening, key) => (
                  <tr key={key} className="intro-x">
                    <td className="font-medium ">{key + 1}</td>
                    <td>
                      <div className="whitespace-nowrap capitalize font-semibold">
                        {rekening.nama_bank ? rekening.nama_bank : "-"} <br />
                        <div className="font-light">
                          {rekening.norek ? rekening.norek : "-"}{" "}
                        </div>
                      </div>
                    </td>
