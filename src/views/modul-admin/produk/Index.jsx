...
function Index() {
  ...
  const { data: produks, isLoading: loading } = useProducts();
  ...
  const filterData = () => {
    return produks?.filter(
      (item) =>
        ...
        item.kode.toLocaleLowerCase().includes(search.toLocaleLowerCase())
    );
  };

  return (
    <>
      <h2 className="intro-y text-lg font-medium mt-10">Produk</h2>
        ...
        <div className="intro-y col-span-12 flex flex-wrap sm:flex-nowrap justify-between mt-2">
          ...
          <div className="w-full sm:w-auto mt-3 sm:mt-0 sm:ml-auto md:ml-0">
            <div className="w-56 relative text-slate-500">
              <input
                type="text"
                className="form-control w-56 produk pr-10"
                placeholder="Search..."
            ...
            <table className="table table-report -mt-2">
              <thead>
                <tr>
                  ...
                  <th className="whitespace-nowrap">Warning Stok Box</th>
                  <th className="whitespace-nowrap">Warning Stok Talang</th>
                  <th className="text-center whitespace-nowrap">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filterData()?.map((produk, key) => (
                  <tr key={key} className="intro-x">
                    <td className="font-medium ">{key + 1}</td>
                    <td>
                      <div className="whitespace-nowrap capitalize font-semibold">
                        {produk.nama_produk ? produk.nama_produk : "-"} <br />
                        <div className="font-light">
                          {produk.kode ? produk.kode : "-"}{" "}
                        </div>
                      </div>
                    </td>
                    <td>
                      <div className="whitespace-nowrap capitalize font-semibold">
                        {produk.kadar ? produk.kadar : "-"}
                      </div>
                    </td>
                    <td>
                      <div className="whitespace-nowrap capitalize font-semibold">
                        {produk.warning_stok_box
                          ? produk.warning_stok_box
                          : "-"}
                      </div>
                    </td>
