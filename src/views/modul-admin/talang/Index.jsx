...
function Index() {
  ...
  const [selectedTalang, setselectedTalang] = useState();
  const [search, setSearch] = useState("");
  const { data: talangs, isLoading: loading } = useTalangs();
  const navigate = useNavigate()
  const handleEdit = (talang) => {
    setselectedTalang(talang);
    setmodalEdit(true);
  };

  const handleDelete = (talang) => {
    setselectedTalang(talang);
    setModalDelete(true);
  };

  function handleDetail(talang) {
    console.log(talang)
    navigate(`/daftar-talang/${talang.id}`)
  }

  const filterData = () => {
    return talangs?.filter((item) =>
  ...
  return (
    <>
      <h2 className="intro-y text-lg font-medium mt-10">Talang</h2>
          ...
          <table className="table table-report -mt-2">
            <thead>
              <tr>
                ...
                <th className="whitespace-nowrap">Nama Talang</th>
                <th className="text-center whitespace-nowrap">Actions</th>
              </tr>
            </thead>
            <tbody>
          
              {filterData()?.map((talang, key) => (
